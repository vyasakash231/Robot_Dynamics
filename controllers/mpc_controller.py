...
class Linear_MPC:
    ...
    |        | = |      |*|     | + |         |*u(t), where Ad = |      |
    |x2(t+dt)|   |0   I | |x2(t)|   |    dt   |                  |0   I |
    """
    def __init__(self, n, x_ref, dt=0.01, N=20, state_cost=5.0, input_cost=1.0):
        ...
        self.Ad = np.block([[np.eye(n, n), dt * np.eye(n, n)],
                            [np.zeros((n, n)), np.eye(n, n)]])  # (2nx2n)
        
        self.Bd = np.block([[((dt**2)/2) * np.eye(n, n)], [dt * np.eye(n, n)]])  # (2nxn)
